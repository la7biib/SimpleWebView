...
(function(jQuery, d, w) {
    ...
    var isMac = w.navigator.platform == 'MacIntel',
        ...
        utils = {
            ...
            html: {
                addTag: function(elem, tag, focus, editable) {
                    var newElement = jQuery(d.createElement(tag));
            ...
            validation: {
                isUrl: function(url) {
                    return (/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?jQuery/).test(url);
                }
            }
        },
        bubble = {
            ...
            updatePos: function(editor, elem) {
                var sel = w.getSelection(),
                    ...
                    pos = {
                        ...
                        y: boundary.top - bubbleHeight - 8 + jQuery(document).scrollTop()
                    };
            ...
            buildMenu: function(editor, elem) {
                ...
                elem.find('button').click(function(e) {
                    e.preventDefault();
                    var cmd = jQuery(this).attr('editor-command');
                ...
                closeBtn.click(function(e) {
                    e.preventDefault();
                    var editor = jQuery(this).closest('.editor');
                    jQuery(this).closest('.link-area').hide();
                    jQuery(this).closest('.bubble').find('ul').show();
                });
            },
            show: function() {
                var tag = jQuery(this).parent().find('.bubble');
                if (!tag.length) {
                    tag = utils.html.addTag(jQuery(this).parent(), 'div', false, false);
                ...
                bubble.updatePos(jQuery(this), tag);
                tag.addClass('active');
            },
            update: function() {
                var tag = jQuery(this).parent().find('.bubble');
            ...
            clear: function() {
                var elem = jQuery(this).parent().find('.bubble');
            ...
            hideButtons: function() {
                jQuery(this).parent().find('.bubble').find('ul').hide();
            },
            showButtons: function() {
                jQuery(this).parent().find('.bubble').find('ul').show();
            },
            showLinkInput: function(selection) {
                bubble.hideButtons.call(this);
                var editor = this;
                var elem = jQuery(this).parent().find('.bubble').find('input[type=text]');
                ...
                var linkText = 'http://';
                if (hasLink) {
                    var anchor = jQuery(utils.selection.getContainer(selection)).closest('a');
                    linkText = anchor.prop('href') || linkText;
                }
                jQuery(this).parent().find('.link-area').show();
                elem.val(linkText).focus();
            },
            hideLinkInput: function() {
                jQuery(this).parent().find('.bubble').find('.link-area').hide();
            }
        },
        actions = {
            ...
            setPlaceholder: function(e) {
                if (/^\s*jQuery/.test(jQuery(this).text())) {
                    jQuery(this).empty();
                    var placeholder = utils.html.addTag(jQuery(this), 'p').addClass('placeholder');
                    placeholder.append(jQuery(this).attr('editor-placeholder'));
                    utils.html.addTag(jQuery(this), 'p', typeof e.focus != 'undefined' ? e.focus : false, true);
                } else {
            ...
            setContentArea: function(elem) {
                var id = jQuery('body').find('.jquery-editor').length + 1;
                elem.attr('data-jquery-notebook-id', id);
                var body = jQuery('body');
                contentArea = jQuery('<textarea></textarea>');
                contentArea.css({
                    position: 'absolute',
                    left: -1000
                });
        ...
        rawEvents = {
            ...
            keyup: function(e) {
                ...
                if (/^\s*jQuery/.test(jQuery(this).text())) {
                    jQuery(this).empty();
                    utils.html.addTag(jQuery(this), 'p', true, true);
                }
            ...
            mouseClick: function(e) {
                ...
                if (jQuery(this).parent().find('.bubble:visible').length) {
                    var bubbleTag = jQuery(this).parent().find('.bubble:visible'),
        ...
        events = {
            commands: {
                ...
                removeLink: function(e, s) {
                    var el = jQuery(utils.selection.getContainer(s)).closest('a');
                    el.contents().first().unwrap();
                    events.change.call(this);
                },
                h1: function(e) {
                    e.preventDefault();
                    if (jQuery(window.getSelection().anchorNode.parentNode).is('h1')) {
                ...
                h2: function(e) {
                    e.preventDefault();
                    if (jQuery(window.getSelection().anchorNode.parentNode).is('h2')) {
                        d.execCommand('formatBlock', false, '<p>');
                    } else {
                ...
                undo: function(e) {
                    ...
                    var sel = w.getSelection(),
                        range = sel.getRangeAt(0),
                        boundary = range.getBoundingClientRect();
                    jQuery(document).scrollTop(jQuery(document).scrollTop() + boundary.top);
                    events.change.call(this);
                }
            },
            enterKey: function(e) {
                if (jQuery(this).attr('editor-mode') === 'inline') {
                    e.preventDefault();
                    e.stopPropagation();
                    return;
                }
                ...
                var elem = jQuery(sel.focusNode.parentElement);
            ...
            paste: function(e) {
                ...
                if (tempArea.length < 1) {
                    var body = jQuery('body');
                    tempArea = jQuery('<textarea></textarea>');
                    tempArea.css({
                        position: 'absolute',
                        left: -1000
                    });
            ...
            change: function(e) {
                var contentArea = jQuery('#jquery-notebook-content-' + jQuery(this).attr('data-jquery-notebook-id'));
                contentArea.val(jQuery(this).html());
    ...
    jQuery.fn.notebook = function(options) {
        options = jQuery.extend({}, jQuery.fn.notebook.defaults, options);
        actions.prepare(this, options);
        actions.bindEvents(this);
        return this;
    };

    jQuery.fn.notebook.defaults = {
        autoFocus: false,
