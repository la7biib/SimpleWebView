...
$("#start_chat").on("mousedown", function(){
initialize();
//alert(chambre);
});
/**
...
openChannel = function() {
  ...
  connection = new WebSocket('ws://46.226.108.135:1234/');
...
maybeStart = function() {
    if (!started && localStream && channelReady) {
        ...
        if (guest)
          {doCall();
		  console.log("Adding local stream11.");
		  }
    }
};
...
onChannelOpened = function() {
    ...
    /*else{
      message = JSON.stringify({"type" : "GETROOM", "value" : $("#nickname").val()});
      console.log(message);
      connection.send(message);
      guest =0;
    }*/
