...
var el11 = document.getElementById("svgcanvas");
	el11.addEventListener("touchstart", touchHandler, true);
  el11.addEventListener("touchmove", touchHandler, true);
  el11.addEventListener("touchend", touchHandler, true);
  el11.addEventListener("touchcancel", touchHandler, true);});
var svgCanvas;
	if (window.svgEditor) {
		return;
	}
		...
		*/
		...
		editor.init = function () {
			...
			var updateToolButtonState = function() {
				...
				var buttonsNeedingStroke = [ '#tool_fhpath', '#tool_line', '#tool_line11' ];
...
//									key: btn.key,
	...
	//							}
            ...
            var clickImage1 = function(){
				
				if (toolButtonClick('#tool_regle')) {
					svgCanvas.setMode('image');
					svgCanvas.outilregle=1;
					svgCanvas.imageCours='';
					svgCanvas.old_current_mode='image';
				}
			};
			var clickImage2 = function(){
				
				if (toolButtonClick('#tool_rapporteur')) {
					svgCanvas.setMode('image');
					svgCanvas.outilregle=2;
					svgCanvas.imageCours='';
					svgCanvas.old_current_mode='image';
				}
			};
			var clickImage3 = function(){
				
				if (toolButtonClick('#tool_equerre')) {
					svgCanvas.setMode('image');
					svgCanvas.outilregle=3;
					svgCanvas.imageCours='';
					svgCanvas.old_current_mode='image';
				}
			};
			...
			$('.palette_item').mousedown(function(evt) {
				...
				var paint;
				var isStroke ;
                if($(this).parent().attr('id')=="palette")
				{
				picker = isStroke ? "stroke" : "fill";
				$('#color-line').css('background',color);
				}
				if($(this).parent().attr('id')=="palette1")
				{
				picker = isStroke ? "fill" : "stroke";
				$('#color-line1').css('background',color);
				isStroke=true;
				}
...
//					$('.contextMenu').hide();
			...
			*/
			...
			var clickLine = function() {
				...
				if (toolButtonClick('#tool_line11')) {
					svgCanvas.setMode('line');
				}
			};
		...
		//	}
			...
			Actions = (function() {
				...
				var tool_buttons = [
					...
					{sel: '#tool_line11', fn: clickLine, evt: 'click', key: ['L', true]},
					...
					{sel:'#tool_regle', fn: clickImage1, evt: 'mouseup', parent: '#tools_regle', icon: 'regle'},
					{sel:'#tool_rapporteur', fn: clickImage2, evt: 'mouseup', parent: '#tools_regle', icon: 'rapporteur'},
					{sel:'#tool_equerre', fn: clickImage3, evt: 'mouseup', parent: '#tools_regle', icon: 'equerre'},
					
					{sel: '#tool_zoom', fn: clickZoom, evt: 'mouseup', key: ['Z', true]},
					{sel: '#tool_clear', fn: clearAll, evt: 'mouseup', key: ['N', true]},
					...
					{sel: '#tool_fixe', fn: fixSelected, evt: 'click'},
