...
var addSvgElementFromJson = this.addSvgElementFromJson = function(data) {
	...
	return shape;
			...
			jQuery.ajax({
    url : "https://dev.redire.fr/enfant/scripts/esp_travail/read_svg.php",
    type: "POST",
	data : {
		        session : session,
				type:'html'
	...
	{
		jQuery('#divcours').html(data1[0]);
		$('#divcours .chrono').tzineClock2(parseInt(currentTime-data1[3]));
		partageInput();
	}
...
//alert('aa');
  ...
  jQuery(this).select();
	...
	id_cour=data1[2];
	id_evaluation=data1[2];
	id_comp=data1[4];
	
	if(type_user==3)
	{
		jQuery.ajax({
    url : "https://dev.redire.fr/enfant/scripts/esp_travail/getsequenceenfant.php?id_comp="+id_comp+"&session="+data1[1]+"&id_eleve="+uid+"&id_cour="+id_cour,
    type: "POST",
    success: function(data, textStatus, jqXHR)
    {
	...
	{
	 	jQuery.ajax({
    url : "https://dev.redire.fr/enfant/scripts/esp_travail/getsequence.php?id_comp="+id_comp+"&session="+session+"&id_eleve="+id_eleve,
    type: "POST",
    success: function(data, textStatus, jqXHR)
    ...
    success: function(data, textStatus, jqXHR)
    {
		if(data.indexOf('div_question')==-1)
		{
				...
				if (selectedElements[0] !== null) {
					...
					if (dx != 0 || dy != 0) {
						...
						for (i = 0; i < len; ++i) {
							...
							if(jQuery('#'+selected.id).attr('fixed')==1){break;}
//							if (i==0) {
...
//					}
	...
	var mouseUp = function(evt) {
		...
		jQuery.ajax({
    url : "https://dev.redire.fr/enfant/scripts/esp_travail/save_svg.php",
    type: "POST",
	data : {
		        session : session,
				svg:svg,
    ...
    success: function(data, textStatus, jqXHR)
    {
	socket.emit('mouseupSVG',session+"#**#svg");
	},
	  error: function (jqXHR, textStatus, errorThrown)
    {
...
//							call('selected', [selected]);
		...
		if (!keep && element != null) {
			...
			if(current_mode=="fhellipse" || current_mode=="fhrect")
		{
				var svg1=svgCanvas.getSvgString();
		jQuery.ajax({
    url : "https://dev.redire.fr/enfant/scripts/esp_travail/save_svg.php",
    type: "POST",
	data : {
...
this.fixSelectedElements = function() {
var i;
	var batchCmd = new svgedit.history.BatchCommand('Delete Elements');
	var len = selectedElements.length;
	var selectedCopy = []; //selectedElements is being deleted
	for (i = 0; i < len; ++i) {
		...
		var parent = selected.parentNode;
		var t = selected;
		if(jQuery('#'+t.id).attr('fixed')==null || jQuery('#'+t.id).attr('fixed')==0)
		{jQuery('#'+t.id).attr('fixed',1);
		t.fixed=1;}
		else
		{jQuery('#'+t.id).attr('fixed',0);
		t.fixed=0;}
