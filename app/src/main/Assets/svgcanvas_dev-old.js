...
var socket=io.connect("https://nodepartage.azurewebsites.net/"); 
...
var current_zoom = 1;
this.svgroot1 = svgdoc.importNode(svgedit.utilities.text2xml(
		'<svg id="svgroot" xmlns="' + svgns + '" xlinkns="' + xlinkns + '" ' +
			'width="' + dimensions[0] + '" height="' + dimensions[1] + '" x="' + dimensions[0] + '" y="' + dimensions[1] + '" overflow="visible">' +
			'<defs>' +
				'<filter id="canvashadow" filterUnits="objectBoundingBox">' +
...
var addSvgElementFromJson = this.addSvgElementFromJson = function(data) {
	...
	socket.on('coursinit'+session,function(d){
	console.log("coursinit");
	
	var data=d.split("#**#");
	if(data[0] != "undefined")
	{jQuery('#divcours').html(data[0]);
	
	$('#divcours .chrono').tzineClock2(parseInt(currentTime-data[3]));
	jQuery("#valid_exercice").html("J’ai fini !");
	
	jQuery("#valid_exercice").attr("onclick","");
	jQuery( "#valid_exercice" ).mousedown(function() {
//alert('aa');
	valid_exercice();
});
	  jQuery( "#divcours input" ).mousedown(function() {
  jQuery(this).select();
  
});

	id_cour=data[1];
	id_evaluation=data[2];
	id_comp=data[4];
	
	if(type_user==3)
	{
		jQuery.ajax({
    url : "getsequenceenfant.php?id_comp="+id_comp+"&session="+session+"&id_eleve="+uid+"&id_cour="+id_cour,
    type: "POST",
    success: function(data, textStatus, jqXHR)
    {
	
	jQuery('#sequence1').html(data);
	...
	{
	 	jQuery.ajax({
    url : "getsequence.php?id_comp="+id_comp+"&session="+session+"&id_eleve="+id_eleve,
    type: "POST",
    success: function(data, textStatus, jqXHR)
    {jQuery('#sequence', window.parent.document).html(data);
	...
	jQuery('#mettrenote').css('display','block');
	 ...
	 //partage select
	 $('#divcours select').change(function(){
	 //alert(this.value);
	 jQuery.each(jQuery('#divcours').find('select'), function(key,val) {
    //alert(key+"  "+jQuery('#cours_interactif').find('select')[key].value);
	if(key==0)
...
{
var evt1=new Object() ;
evt1.eventInit =evt1.eventInit ;
evt1.bubbles	=evt.bubbles	;
evt1.cancelable	=evt.cancelable	;
//evt1.view	=evt.view	;
evt1.detail	=evt.detail	;
evt1.screenX	=evt.screenX+$("#workarea").scrollLeft()	;
evt1.screenY	=evt.screenY+$("#workarea").scrollTop()	;
evt1.clientX	=evt.clientX+$("#workarea").scrollLeft()	;
evt1.clientY	=evt.clientY+$("#workarea").scrollTop()	;
evt1.pageX	=evt.pageX+$("#workarea").scrollLeft()	;
evt1.pageY	=evt.pageY+$("#workarea").scrollTop()	;
evt1.ctrlKey	=evt.ctrlKey	;
evt1.shiftKey	=evt.shiftKey	;
evt1.altKey	=evt.altKey	;
evt1.metaKey	=evt.metaKey	;
evt1.button	=evt.button	;
evt1.buttons	=evt.buttons	;
evt1.relatedTarget	=evt.relatedTarget;
evt1.targetId	=evt.target.id;
evt1.current_mode=current_mode;
evt1.session=session;
//console.log(evt.target);
//JSON.stringify(evt.target)
return JSON.stringify(evt1);	
}
			...
			case "select":
				...
				if (mouse_target != svgroot) {
					...
					if(!right_click) {
						...
						if(jQuery("#"+selectedElements[0].id).attr('block')==1)
				{
				jQuery( "#fixer" ).fadeOut();
	jQuery( "#defixer" ).fadeIn();
				
				}
				else
				{
				jQuery( "#fixer" ).fadeIn();
	jQuery( "#defixer" ).fadeOut();
				}
				}
				else if(!right_click){
					clearSelection();
...
// 					console.log('s',[start_x, start_y]);
			...
			case "image":
				...
				if(canvas.outilregle == 1)
				last_good_img_url="images/regle.png";
				if(canvas.outilregle == 2)
				last_good_img_url="images/rapporteur.png";
				if(canvas.outilregle == 3)
				last_good_img_url="images/equerre.png";
				if(canvas.imageCours!='')
				last_good_img_url=canvas.imageCours;
...
// 					newText.textContent = "text";
	...
	//socket.emit('pushbutton',this.id);
	});
	
	socket.on('pushbutton1'+session,function(d){
	//$('#'+ d).click();
	var data=d.split("#**#");
	alert(data[1]);
	jQuery("#"+data[1]).attr('block',data[0]);
	});
	//alert(session);

		socket.on('reponseinit'+session,function(d){
		console.log(d);
		if(d!=null)
		{var data=d.split("#**#");
		if(data[2]=="text"){
	...
	//$('#rep'+i).val(rep[i-1]);
	jQuery('#divcours input:text').eq(i-1).val(rep[i-1]);
	}
	}
	if(data[2]=="radio"){
	console.log('r');
	...
	for(var i=1; i<=rep.length;i++){
	console.log(rep[i-1]);
	//$('#'+rep[i-1]).attr('checked', true);
	jQuery('#divcours input:text').eq(i-1).attr('checked', true);
	}
	}
	if(data[2]=="checkbox"){
	console.log('r');
	...
	for(var i=1; i<=rep.length;i++)
	{
	 if(rep[i-1]=='1'){
	 console.log(rep[i-1]);
	 jQuery('#divcours input:text').eq(i-1).attr('checked','checked');
	 //$('#rep'+i).attr('checked','checked');
	 }
	 else {
	 jQuery('#divcours input:text').eq(i-1).removeAttr("checked")
	// $('#rep'+i).removeAttr("checked");
	 }
	}
	...
	{
	 console.log(rep[i-1]);
	 jQuery('#divcours select').eq(i).val(rep[i]);
	}
	}
	}
	if(data)
	...
	socket.on('reponse1'+session,function(d){
      //alert(d);
	var data=d.split("#**#");
	console.log(data[3]);
	if(data[3]=="checkbox"){
	...
	{
	 console.log(rep[i-1]);
	 //$('#'+i+' option[value="'+rep[i-1]+'"]').prop('selected', true);
	 jQuery('#divcours select').eq(i).val(rep[i]);
	}
	}
	});
	//start_eval
	var setIntervalChrono1;
	socket.on('start_eval1'+session,function(d){
	
	var seconds = 30;
	setIntervalChrono1=setInterval(function () {
 
    $('.tab_intro .chrono').tzineClock1(seconds);
    seconds--;
     if(seconds==0)
       {
	   clearInterval(setIntervalChrono1);
	   }
}, 1000);
	});
	//Partage de cours
	
	socket.on('cours1'+session,function(d){
	
	var svgclear='<svg width="1400" height="1400" xmlns="http://www.w3.org/2000/svg">'+
 '<g>'+
  '<title>Layer 1</title>'+
 '</g>'+
'</svg>';
...
//alert('aa');
  ...
  jQuery(this).select();
	...
	id_cour=data[2];
	jQuery('#sequence1 .btn').removeClass('btn-encours');
	jQuery('#exo_enfant_'+id_cour).addClass('btn-encours');
	
	
	if(data[3]=="intro")
	{
	id_evaluation=data[4];
	if(setIntervalChrono1)
clearInterval(setIntervalChrono1);
...
socket.on('mouseupSVG1'+session,function(d){
//alert(d);
console.log(d);
svgCanvas.setSvgString(d);
});
socket.on('mouseupSVG2'+session,function(d){

svgCanvas.setSvgString(d);
socket.emit('mouseupSVG',session+"#**#"+d);

});


socket.on('addimage1'+session,function(d){
...
var newImage = svgCanvas.addSvgElementFromJson({
	...
	jQuery( "#defixer" ).fadeOut();
				...
				if (selectedElements[0] !== null && jQuery("#"+selectedElements[0].id).attr('block')!=1) {
					var dx = x - start_x;
					var dy = y - start_y;
...
//							}
			...
			case "resize":
				...
				console.log('aaaaa');
				if(jQuery("#"+selected.id).attr('block')!=1)
				{
				var tlist = getTransformList(selected),
...
// 					}
    ...
    if ( ((screen.width  >= 480) && (screen.height >= 800)) || ((screen.width  >= 800) && (screen.height >= 480)) || navigator.userAgent.match(/ipad/gi) ) {
       // alert('tablette');
	 $('#tool_delete').mousedown(function(){
...
var newImage = addSvgElementFromJson({
				...
				last_good_img_url=this.imageCours;
...
this.setMode = function(name) {
	...
	current_mode = name;
	//old_current_mode=name;
};
this.old_current_mode=current_mode;
...
this.blockSelected = function() {
var selected = selectedElements[0];
	if (!selected) return;
	//jQuery(selected).addClass('block');
	console.log(selected);
   jQuery("#"+selected.id).attr('block',1);
   socket.emit('pushbutton',"1#**#"+selected.id+"#**#"+session);
}
this.unblockSelected = function() {
var selected = selectedElements[0];
	if (!selected) return;
	//jQuery(selected).addClass('block');
	
	jQuery("#"+selected.id).attr('block',0);
	socket.emit('pushbutton',"0#**#"+selected.id+"#**#"+session);
   //alert(selected.block);
}
